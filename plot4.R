...
par(mfrow=c(2,2))
par(mar=c(4,4,1,1))
plot(df$dt_value,df$Global_active_power, type="l",xlab="",ylab="Global Active Power")
plot(df$dt_value,df$Voltage, type="l", xlab="Datetime", ylab="Voltage")
...
legend("top", lty=c(1,1,1), col=c("black","red","blue"), legend=names(df[,7:9]),xjust=-1,cex=0.6,y.intersp=1,bty="n")
plot(df$dt_value,df$Global_reactive_power, type="l",xlab="Datetime", ylab="Global_reactive_power")
